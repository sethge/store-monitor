...
如果执行过程中遇到缺少依赖的报错（如 `Cannot find package 'sharp'`、`No module named 'xxx'`、`ffmpeg: command not found`），**不要让运营自己去查，直接帮他修**：

```bash
bash skills/store-diagnosis/setup.sh
```

这个脚本会自动检测并安装所有缺失的依赖（Node.js/sharp/Python/ffmpeg/xlsxwriter/lzstring）。装完后重新执行即可。

如果 setup.sh 也失败，告诉运营具体哪个装不上，给出一行命令让他复制粘贴执行。
...
5. **环境问题不能甩给运营。** 缺依赖就跑setup.sh自动装，不让运营自己研究。
