...
if [ ! -d "$HOME/wisdom-brain" ]; then
    GIT_TERMINAL_PROMPT=0 git clone --quiet https://gitee.com/sethgeshiheng/wisdom-brain.git "$HOME/wisdom-brain" 2>/dev/null && \
        echo "  ✓ wisdom-brain 已克隆" || \
        echo "  ⏭ wisdom-brain 跳过（不影响基础功能）"
else
    GIT_TERMINAL_PROMPT=0 git -C "$HOME/wisdom-brain" pull --quiet 2>/dev/null
